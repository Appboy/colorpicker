  ...
  let ColorPicker = (function() {
    var ids = {},
      ...
      upIncrement = function(ev) {
        change.apply(ev.data.field.get(0), [true]);
        ev.data.el
          .removeClass("colorpicker_slider")
          .find("input")
          .focus();
      ...
      upSelector = function(ev) {
        // Appboy Modification - call moveSelector here so clicking without dragging will change the color
        moveSelector(ev);
        fillRGBFields(ev.data.cal.data("colorpicker").color, ev.data.cal.get(0));
        fillHexFields(ev.data.cal.data("colorpicker").color, ev.data.cal.get(0));
      ...
      init: function(opt) {
        ...
        return this.each(function() {
          if (!$(this).data("colorpickerId")) {
            ...
            if (options.flat) {
              // Appboy Modification: use position: absolute rather than position: relative so that the
              // colorpicker doesn't displace other elements on the page.
              cal.css({
                position: "absolute",
                display: "block",
              });
